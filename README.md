<div align="center">
  <a href="https://finswitch.pages.dev">
    <img src="https://raw.githubusercontent.com/OK45batwal/FINSWITCH/master/website/public/logo.svg" alt="FinSwitch Logo" width="100" height="100">
  </a>
...
graph TD
    ...
    subgraph Edge[" Edge & Middleware Layer "]
        ...
        CFFun["⚡ Cloudflare Function (/api/ai)"]
    end

    subgraph Backend[" Data & AI Layer "]
        Supa[("⚡ Supabase PostgreSQL & Auth")]
        AI["🤖 LLM Financial Engine"]
    end

    Web -->|Edge Routing & SSG| CF
    Mobile -->|HTTPS / REST| Supa
    Web -->|Database & Auth| Supa
    Web -->|AI Prompts| CFFun
    Mobile -->|AI Prompts| CFFun
    CFFun -->|Context-Grounded Query| AI
```
...
sequenceDiagram
    ...
    Dev->>GH: Execute release script (scripts/release.sh v1.x.x)
    GH->>GHA: Trigger release workflow on push tag v*
    GHA->>Release: Build release APK & attach binary to release tag
    App->>GH: App check for update (fetches raw pubspec.yaml)
    alt New Version Available
        GH-->>App: Return latest version tag
        App->>Release: Auto-download app-release.apk
        App->>Dev: Prompt user to tap Install via System Package Installer
    end
```
