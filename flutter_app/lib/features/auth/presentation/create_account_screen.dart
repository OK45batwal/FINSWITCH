...
class _CreateAccountScreenState extends State<CreateAccountScreen> {
  ...
  Future<void> _verifyOtp() async {
    ...
    if (otp.isEmpty) {
      setState(() => _otpError = 'Please enter the OTP code');
      return;
    }
    setState(() {
      _otpVerified = true;
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          if (_isRegisterMode)
            TextButton(
              onPressed: _isOtpMode ? _sendOtp : _forgotPassword,
              child: Text(_isOtpMode ? 'Send OTP' : 'Forgot password?'),
            ),
        ],
      ),
      body: SafeArea(
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                    ...
                    children: [
                      Expanded(
                        child: GestureDetector(
                          onTap: () => setState(() { _isRegisterMode = false; _error = null; _isOtpMode = false; _otpSent = false; _otpVerified = false; _otpCtl.clear(); }),
                          child: Container(
                      ...
                      Expanded(
                        child: GestureDetector(
                          onTap: () => setState(() { _isRegisterMode = true; _error = null; _isOtpMode = false; _otpSent = false; _otpVerified = false; _otpCtl.clear(); }),
                          child: Container(
                            padding: const EdgeInsets.symmetric(vertical: 10),
                            decoration: BoxDecoration(
                              color: _isRegisterMode ? Theme.of(context).colorScheme.surface : Colors.transparent,
                ...
                if (_isOtpMode) ...[
                  Text(
                    'Enter the OTP code sent to ${_emailCtl.text.trim()}',
                  ...
                  const SizedBox(height: 12),
                  ElevatedButton(
                    onPressed: _otpVerified ? _verifyOtp : _sendOtp,
                    child: Text(_otpVerified ? 'Verify OTP' : 'Send OTP'),
                  ),
                ],
                const SizedBox(height: 24),
                SizedBox(
